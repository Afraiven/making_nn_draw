...
def rnn_model_prediction(points):
    # Load model from 'rnn_model.h5'
    model = tf.keras.models.load_model('../models/rnn_model.h5', custom_objects={'mae': mae})
    # Predict
    X = np.array([points])
...
class FastInteractiveDrawingBoard(FastDrawingBoard):
    ...
    def on_release(self, event):
        ...
        if event.button == 1:  # Left mouse button
            ...
            # # Remove drawing from board
            self.drawing = False
            self.last_pos = None
            # Collect drawn points
            points = []
            points_to_print = []
            drawn_points = np.argwhere(self.board == 0)
            self.board[self.board == 0] = 255  # Reset board
            step = len(drawn_points) ** 0.8
            for i in range(0, len(drawn_points), int(step)):
                points_to_print.append(drawn_points[i][::-1])  # Reverse (y, x) to (x, y)
                points.append(drawn_points[i][::-1])  # Reverse (y, x) to (x, y)
            ...
            for j in range(50):
                ...
                plt.xlim(0, 255)
                plt.ylim(0, 255)  # Ensure the Y-axis is correctly oriented (origin at bottom-left)
                plt.gca().invert_yaxis()  # Invert Y-axis to match drawing board orientation
                plt.draw()
                plt.pause(0.01)

            plt.xlim(0, 255)
            plt.ylim(0, 255)
            plt.gca().invert_yaxis()  # Keep Y-axis inverted for final display
            plt.show()
        self.update_display()
